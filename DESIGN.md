...
- Direction: `--positive`, `--negative` (`#C8372D` light)
- `--primary-foreground` is Paper in both themes: emerald fills carry white ink. Carbon on
  ...
  floor, taken for the fintech read on a single-user app.
...
- `<CellMeter>` — a fraction of a known limit as a run of cells, for budgets. Not for
  accounts: measuring each against the largest one ranks them and says nothing about how the
  whole divides, which is the question the Balances card is asked.
